...
class SmartTranslator:
    ...
    def restore_placeholders(self, text: str, ph_map: dict[str, str]) -> str:
        ...
        for token, original in ph_map.items():
            inner = token.strip(" []-")
            pattern = re.compile(
                rf"\[\[\s*-*\s*{re.escape(inner)}\s*-*\s*\]\]", re.IGNORECASE
            )
...
class ScanVisitor(ast.NodeVisitor):
    ...
    def visit_Constant(self, node: ast.Constant):
        ...
        technical_patterns = [
            ...
            r"<.*?>",
            r"^IDENTITY:",
            r"^\d+ (MB|GB|KB|week|day|month|year)s?$",
            r"\{time:.*?\}",
            r"^https?://",
            r"^[a-z0-9_\-\.]+\.[a-z]{2,4}(/.*)?$",
        ]
        ...
        line_content = (
            self.lines[line_no - 1].strip() if 0 < line_no <= len(self.lines) else ""
        )

        if any(
            x in line_content.lower() for x in ["print(", "raise ", "filters.command"]
        ):
            return
...
def scan_unlocalized(mgr: LocaleManager):
    ...
    for folder in ["plugins", "utils", "db", "filters", "custom_filters"]:
...
def main():
    ...
    parser.add_argument(
        "--sort", action="store_true", help="Only sort files and sync keys"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--scan", action="store_true", help="Scan for unlocalized strings in src/"
    )
    parser.add_argument(
        "--unused", action="store_true", help="Detect unused localization keys"
    )
    parser.add_argument(
        "--prune",
        action="store_true",
        help="Prune extra keys from locales without translating",
    )
    ...
    if not args.sort:
        ...
        for lang_name in target_langs:
            ...
            missing = [
                k for k in mgr.en_data if k not in data or args.force or k in force_keys
            ]
    ...
    if args.check:
        for job in work_plan:
            ...
            if job["missing"]:
                print(
                    f"  {Fore.YELLOW}∆ Found {len(job['missing'])} keys needing translation."
                )
        return
