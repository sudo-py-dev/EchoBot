...
class Translator:
    ...
    def _restore(self, text: str, mapping: dict[str, str]) -> str:
        ...
        def replace_token(match: re.Match) -> str:
            idx = match.group(1)
            return mapping.get(
                PLACEHOLDER_TOKEN.format(idx), match.group(0)
            ) or match.group(0)
...
class StringScanner(ast.NodeVisitor):
    ...
    def __init__(
        self, en_keys: set[str], en_values: set[str], content: str, rel_path: str
    ):
...
def scan_unlocalized(mgr: LocaleManager) -> int:
    ...
    console.print(
        Panel("[cyan]Scanning for unlocalized strings in src/...", title="Scan")
    )
...
def find_unused_keys(mgr: LocaleManager) -> list[str]:
    ...
    console.print(
        Panel("[cyan]Scanning for unused localization keys...", title="Unused Keys")
    )
...
def build_work_plan(
    ...
    for lang in target_langs:
        ...
        missing = [
            k
            for k in mgr.en_data
            if k not in data or force or (force_keys and k in force_keys)
        ]
...
def translate_job(
    mgr: LocaleManager, job: dict, progress: Progress, task_id: int
) -> None:
    ...
    if not missing:
        if job["extra"]:
            console.print(
                f"[red]➤ {job['name'].upper()}: Pruned {len(job['extra'])} extra keys."
            )
    ...
    for i in range(0, len(to_translate), CHUNK_SIZE):
        ...
        progress.update(
            task_id,
            advance=0,
            description=f"[cyan]Translating {job['name'].upper()}[/]",
        )
...
def sort_all_locales(
    mgr: LocaleManager, target_langs: list[str], dry_run: bool = False
) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--langs", help="Target languages (comma-separated, e.g., he,ru,es)"
    )
    parser.add_argument(
        "--sort",
        action="store_true",
        help="Sort all locale files to match en.json key order",
    )
    parser.add_argument(
        "--check", action="store_true", help="Check differences without translating"
    )
    parser.add_argument(
        "--force", action="store_true", help="Force re-translation of all keys"
    )
    parser.add_argument(
        "--keys", help="Specific keys to force translate (comma-separated)"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Enable verbose output"
    )
    parser.add_argument(
        "--scan", action="store_true", help="Scan for unlocalized strings"
    )
    ...
    parser.add_argument(
        ...
        "-l",
        action="store_true",
        help="Show translation statistics for all locales",
    )
    parser.add_argument(
        "--dry-run",
        "-n",
    ...
    parser.add_argument(
        "--create", help="Create new locale file(s) for specified language(s)"
    )
    ...
    if args.list:
        all_langs = sorted(
            [f.stem for f in LOCALES_DIR.glob("*.json") if f.name != "en.json"]
        )
    ...
    if args.create:
        ...
        for lang in new_langs:
            ...
            if not args.dry_run:
                ...
                created += 1
            console.print(
                f"  [green]✓ {'Would create' if args.dry_run else 'Created'} {lang}.json"
            )
        if created > 0:
            console.print(
    ...
    if total_missing == 0 and any(j["extra"] for j in plan):
        ...
        for job in plan:
            ...
            mgr.save_json(job["path"], data)
            console.print(
                f"[green]✓ {job['name'].upper()}: Pruned {len(job['extra'])} extra keys."
            )
        return
